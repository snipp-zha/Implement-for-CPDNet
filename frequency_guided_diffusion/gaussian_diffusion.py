...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        # print('self.model_var_type is',self.model_var_type) # LEARNED_RANGE
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            if self.model_var_type == ModelVarType.LEARNED:
                ...
                max_log = _extract_into_tensor(np.log(self.betas), t, x.shape) # 1055行
    ...
    def _predict_xstart_from_eps(self, x_t, t, eps): # ([1, 1, 256, 256]) ([1, 2, 256, 256]) 大小不相等
    ...
    def p_sample(
        ...
        """
        out = self.p_mean_variance( #跳转231行
            model,
            x,
            t,
    ...
    def p_sample_loop_known(
        ...
        #add noise as the last channel 变成了3个通道，-1不会取到最后一个通道
        # img=img.to(device) #应该是x_noisy
    ...
    def p_sample_loop_progressive(
        ...
        if noise is not None:
            img = noise # 原图像和噪声在通道维度
        else:
            img = th.randn(*shape, device=device) # 噪声是原图像在通道数conca一张噪声图像， img的形状有问题？
        indices = list(range(time))[::-1] # 1000
        ...
        if progress:
           ...
           for i in indices:
                ...
                with th.no_grad():
                    ...
                    yield out
                    img = out["sample"] # 选了其中一个输出，输出有两个通道
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, subtra, model_kwargs=None, noise=None):
        """
        ...
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
            noise = th.randn_like(x_start[:, -1:, ...]) # 一个通道


        mask = x_start[:, -1:, ...]
        res = torch.where(mask > 0, 1, 0)   #标签
        subtra = subtra.squeeze(1)
        res_t = self.q_sample(res, t, noise=noise)    #对分割金标准加噪声 190行
        subtra = self.q_sample(subtra, t, noise=noise)
        #print(subtra.shape)
        x_t=x_start.float()
        # x_t[:, -1:, ...]=res_t.float() #把加了噪声的标签和输入图像concatenate,换成sub
        x_t[:, -1:, ...]=subtra.float()
        terms = {}


        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE: #LossType.MSE
            ...
            if self.model_var_type in [
                ...
                # print('self.model_var_type is', self.model_var_type)
                B, C = x_t.shape[:2]
                C=1
